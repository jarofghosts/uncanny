...
function renderBlogs(uncanny, callback) {
  fs.readFile(uncanny.source + 'blogs/layout.ejs', 'utf8', function (err, template) {
    uncanny.uncanny.blogs.forEach(function (blog) {
      ...
      fs.writeFileSync(uncanny.target + 'blogs/' + blog.name + '.html', blogPage);
    });
    callback && callback();
  });
}
        ...
        files.forEach(function (file) {
          ...
          if (blogDate) {
            var blogName = file.replace(uncanny.blogDateRegEx, '').substring(1).replace(/\.md$/i, ''),
              contentRendered = fs.existsSync(uncanny.target + 'blogs/' + blogName + '.htm'),
              blogContent = contentRendered ? fs.readFileSync(uncanny.target + 'blogs/' + blogName + '.htm', 'utf8') : '',
              blogObject = {
                "year": blogDate[1],
                "month": blogDate[2],
                "day": blogDate[3],
            ...
            if (!contentRendered) { renderBlogs(uncanny, function () { rebuildUncanny(uncanny); }); }
          }
        });
